...
function render(timeStamp) {
    ...
    if (orbiting) {
        ...
        //console.log(V)
        //console.log(mult(V, vec4(0.0, 0.0, -1.0, 0.0)))
        let VRot = mat3(
            [V[0][0], V[0][1], V[0][2]],
            [V[1][0], V[1][1], V[1][2]],
            [V[2][0], V[2][1], V[2][2]]
        );
        gl.uniform3fv(lightPositionLoc, mult(VRot, vec3(0.0, 0.0, -1.0)));
    }
...
window.onload = function init() {
    ...
    var emissionLLoc = gl.getUniformLocation(program, "emissionL");
    gl.uniform1f(emissionLLoc, 1.0);

    var ambientKLoc = gl.getUniformLocation(program, "ambientK");
    gl.uniform1f(ambientKLoc, 0.5);

    var diffuseKLoc = gl.getUniformLocation(program, "diffuseK");
    gl.uniform1f(diffuseKLoc, 0.5);

    var specularKLoc = gl.getUniformLocation(program, "specularK");
    gl.uniform1f(specularKLoc, 0.5);

    var shininessLoc = gl.getUniformLocation(program, "shininess");
    gl.uniform1f(shininessLoc, 100.0);
    ...
    function addSliderCallback(name, toUpdate, uniformLoc) {
    ...
    addSliderCallback("ambientK", ambientK, ambientKLoc);
    addSliderCallback("diffuseK", diffuseK, diffuseKLoc);
    addSliderCallback("specularK", specularK, specularKLoc);
    addSliderCallback("shininess", shininess, shininessLoc);
    addSliderCallback("lightEmission", lightEmission, emissionLLoc);
